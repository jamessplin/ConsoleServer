...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Synchronize explicitly mapped ConsoleServer files from the real "
            "SONiC tree to the tracking repository."
        )
    )
    parser.add_argument(
        "--tracking-root",
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Perform copies. Without this option, show a dry run.",
    )
...
def main() -> int:
    ...
    for item in MAPPINGS:
        source = sonic_root / item.sonic
        target = tracking_root / item.tracking

        if not source.is_file():
            raise SystemExit(f"ERROR: SONiC source file missing: {source}")

        differs = (
            not target.is_file()
            or sha256(source) != sha256(target)
            or executable_bits(source) != executable_bits(target)
        )
        if differs:
            planned.append((item, source, target))

    obsolete = [
        tracking_root / relative
        for relative in OBSOLETE_TRACKING_PATHS
        if (tracking_root / relative).exists()
    ]

    if not args.apply:
        for item, _, _ in planned:
            print(f"WOULD UPDATE: {item.tracking}")
        for path in obsolete:
            print(f"WOULD REMOVE OBSOLETE: {path.relative_to(tracking_root)}")
        print()
        print(f"Dry run: {len(planned)} update(s), {len(obsolete)} removal(s)")
        print("Run again with --apply to make these changes.")
        return 0

    for item, source, target in planned:
        result = copy_verified(source, target)
        print(f"{result}: {item.tracking}")

    for path in obsolete:
        if not path.is_file():
            raise SystemExit(f"ERROR: obsolete path is not a file: {path}")
        path.unlink()
        print(f"REMOVED OBSOLETE: {path.relative_to(tracking_root)}")

    print()
    print("PASS: SONiC-to-tracking synchronization completed")
    return 0
