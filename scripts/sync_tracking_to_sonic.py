...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Synchronize explicitly mapped ConsoleServer files from the "
            "tracking repository to the real SONiC tree."
        )
    )
    parser.add_argument(
        "--tracking-root",
...
def main() -> int:
    ...
    for item in MAPPINGS:
        source = tracking_root / item.tracking
        target = sonic_root / item.sonic

        if not source.is_file():
            raise SystemExit(f"ERROR: tracking source file missing: {source}")
    ...
    if not args.apply:
        for item, _, _ in planned:
            print(f"WOULD UPDATE: {item.sonic}")
        print()
        print(f"Dry run: {len(planned)} update(s)")
        print("Run again with --apply to make these changes.")
        print("Tracking-only files are intentionally not copied.")
        return 0

    for item, source, target in planned:
        result = copy_verified(source, target)
        print(f"{result}: {item.sonic}")

    print()
    print("PASS: tracking-to-SONiC synchronization completed")
    return 0
